...
def download_files(filenames, base_url, output_path, sample_only=False, pool=None):
    ...
    for i, filename in enumerate(filenames):
        ...
        if pool is not None:
            ...
            all_task.append(pool.submit(download_file, url, path))
        if sample_only and i != 0 and i % (NUM_SAMPLES - 1) == 0:
            break

    for i, future in enumerate(as_completed(all_task)):
        future.result()
        ...
        for folder in downloaded_folders:
            ...
            for video_type in downloaded_video_types:
                ...
                download_files(filenames, base_url, output_path=output_path,
                               sample_only=args.sample_only, pool=executor)
