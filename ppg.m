...
function finalPPG = compute_ippg(rawColorSignal, ippgSettings)
  ...
  if ippgSettings.extractionMethod == ippgSettings.EXTRACTION.ICA
...
function ppgSign = calc_ppg_sign(ppg)
  ...
  ppg(ppg == 0) = 0.00001; % get read of exact zeros
  ...
  for iSegment = 1:nSegment
    sumMax = sumMax + max(indexUp(iSegment):indexDown(iSegment) );
    sumMin = sumMin - min(indexDown(iSegment):indexUp(iSegment+1)) ;
  end
